import {getCarts, addOne, getCartById, addProdToCart, updateProductQuantity, emptyCart, delProdFromCart, deleteCart} from '../services/carrito.services.js';
...
export async function addProdsToCart(req, res){
    try {
        ...
        res.json({message: 'product added', newCart: cartMod})
    } catch (error) {
        res.status(500).json(error)
    }
}
...
export async function updateProductsQuantity(req,res){
    try {
        ...
        res.json({message: "product updated", updatedCart})
    } catch (error) {
        res.status(500).json(error)
    }
}
